...
bool setNormalUnitNormal(Point before, Point after, float scale, float radius,
    ...
    if (!unitNormal.setNormalize((after.x - before.x) * scale,
...
ReductionType checkCubicLinear(const Point cubic[4], Point reduction[3],
    ...
    NormalizedF32 tValues[3] = {NormalizedF32::ZERO, NormalizedF32::ZERO,
                                NormalizedF32::ZERO};
    ...
    for (std::size_t i = 0; i < tSlice; ++i) {
        ...
        if (!(reduction[rCount] == cubic[0]) &&
            !(reduction[rCount] == cubic[3])) {
            rCount += 1;
        }
    }
...
std::size_t intersectQuadRay(const Point line[2], const Point quad[3],
    ...
    float r[3];
    for (int n = 0; n < 3; ++n) {
        r[n] = (quad[n].y - line[0].y) * vec.x -
               (quad[n].x - line[0].x) * vec.y;
    }
    float a = r[2];
    float b = r[1];
    float c = r[0];
...
void roundCapper(Point pivot, Point normal, Point stop,
    ...
    path.conicPointsTo(projectedCenter + normal, projectedCenter,
                       kScalarRoot2Over2);
...
void squareCapper(Point pivot, Point normal, Point stop,
    ...
    if (otherPath != nullptr) {
        path.setLastPoint(Point::fromXy(pivot.x + normal.x + parallel.x,
...
void roundJoiner(Point beforeUnitNormal, Point pivot, Point afterUnitNormal,
    ...
    Transform ts =
        Transform::fromRow(radius, 0.0f, 0.0f, radius, pivot.x, pivot.y);

    path_geometry::Conic conics[5];
    auto conicsSpan =
        path_geometry::Conic::buildUnitArc(before, after, dir, ts, conics);
    if (conicsSpan.has_value()) {
        for (const auto& conic : conicsSpan.value()) {
            builders.outer->conicPointsTo(conic.points[1], conic.points[2],
                                          conic.weight);
        }
        after.scale(radius);
        handleInnerJoin(pivot, after, *builders.inner);
...
static void doBluntOrClipped(SwappableBuilders builders, Point pivot,
                              float radius, bool prevIsLine, bool currIsLine,
                              Point before, Point mid, Point after,
...
static void doMiter(SwappableBuilders builders, Point pivot, float radius,
                    bool prevIsLine, bool currIsLine, Point mid, Point after) {
    after.scale(radius);

    if (prevIsLine) {
        builders.outer->setLastPoint(
            Point::fromXy(pivot.x + mid.x, pivot.y + mid.y));
    } else {
...
static void miterJoinerInner(Point beforeUnitNormal, Point pivot,
    ...
    if (angleType == AngleType::Nearly180) {
        ...
        doBluntOrClipped(builders, pivot, radius, prevIsLine, currIsLine,
                         before, mid, after, invMiterLimit, miterClip);
        return;
    }
...
void miterJoiner(Point beforeUnitNormal, Point pivot, Point afterUnitNormal,
                 float radius, float invMiterLimit, bool prevIsLine,
                 bool currIsLine, SwappableBuilders builders) {
    miterJoinerInner(beforeUnitNormal, pivot, afterUnitNormal, radius,
                     invMiterLimit, /*miterClip=*/false, prevIsLine, currIsLine,
                     builders);
}
...
void miterClipJoiner(Point beforeUnitNormal, Point pivot,
                     ...
                     SwappableBuilders builders) {
    miterJoinerInner(beforeUnitNormal, pivot, afterUnitNormal, radius,
                     invMiterLimit, /*miterClip=*/true, prevIsLine, currIsLine,
                     builders);
}
...
std::optional<Path> PathStroker::stroke(const Path& path, const Stroke& stroke,
    ...
    return strokeInner(path, stroke.width, stroke.miter_limit, stroke.line_cap,
                       stroke.line_join, resolutionScale);
}

std::optional<Path> PathStroker::strokeInner(const Path& path, float width,
                                              float miterLimit,
                                              LineCap lineCap,
                                              LineJoin lineJoin,
                                              float resScale) {
...
SwappableBuilders PathStroker::builders() {
    return SwappableBuilders{&inner_, &outer_};
}

Point PathStroker::moveToPt() const {
    return firstPt_;
}
...
void PathStroker::lineTo(Point p, const PathSegmentsIter* iter) {
    bool teenyLine = prevPt_.equalsWithinTolerance(
        p, kScalarNearlyZero * invResScale_);
    ...
    if (teenyLine &&
        (joinCompleted_ ||
         (iter != nullptr && iter->hasValidTangent()))) {
        return;
    }
...
void PathStroker::quadTo(Point p1, Point p2) {
    ...
    initQuad(StrokeType::Outer, NormalizedF32::ZERO, NormalizedF32::ONE,
             quadPoints);
    quadStroke(quad, quadPoints);
    initQuad(StrokeType::Inner, NormalizedF32::ZERO, NormalizedF32::ONE,
             quadPoints);
    quadStroke(quad, quadPoints);

    bool ok = setNormalUnitNormal(quad[1], quad[2], resScale_, radius_,
                                  normalBc, unitBc);
    if (!ok) {
        normalBc = normalAb;
        unitBc = unitAb;
...
void PathStroker::cubicTo(Point pt1, Point pt2, Point pt3) {
    ...
    ReductionType reductionType =
        checkCubicLinear(cubic, reduction, &tangentPt);
    if (reductionType == ReductionType::Point) {
        lineTo(pt3, nullptr);
        return;
    }
    ...
    if (reductionType >= ReductionType::Degenerate &&
        reductionType <= ReductionType::Degenerate3) {
    ...
    std::size_t tCount =
        path_geometry::findCubicInflections(cubic, tValuesStorage);
    ...
    if (cusp.has_value()) {
        Point cuspLoc =
            path_geometry::evalCubicPosAt(cubic, cusp->toNormalized());
...
bool PathStroker::cubicStroke(const Point cubic[4],
                               QuadConstruct& quadPoints) {
    if (!foundTangents_) {
        ResultType resultType = tangentsMeet(cubic, quadPoints);
        if (resultType != ResultType::Quad) {
            bool ok = pointsWithinDist(quadPoints.quad[0], quadPoints.quad[2],
                                       invResScale_);
            if ((resultType == ResultType::Degenerate || ok) &&
                cubicMidOnLine(cubic, quadPoints)) {
    ...
    if (foundTangents_) {
        ...
        if (resultType == ResultType::Quad) {
            ...
            if (strokeType_ == StrokeType::Outer) {
                outer_.quadTo(stroke[1].x, stroke[1].y, stroke[2].x,
                              stroke[2].y);
            } else {
                inner_.quadTo(stroke[1].x, stroke[1].y, stroke[2].x,
                              stroke[2].y);
            }
            return true;
        }
    ...
    if (!std::isfinite(quadPoints.quad[2].x) ||
        !std::isfinite(quadPoints.quad[2].y)) {
...
bool PathStroker::cubicMidOnLine(const Point cubic[4],
                                  QuadConstruct& quadPoints) {
...
void PathStroker::cubicQuadMid(const Point cubic[4],
                                QuadConstruct& quadPoints, Point& mid) {
    Point cubicMidPt = Point::zero();
    cubicPerpRay(cubic, quadPoints.midT, cubicMidPt, mid, nullptr);
}

void PathStroker::cubicPerpRay(const Point cubic[4], NormalizedF32 t,
                                Point& tPt, Point& onPt, Point* tangent) {
...
void PathStroker::setCubicEndNormal(const Point cubic[4], Point normalAb,
...
ResultType PathStroker::compareQuadCubic(const Point cubic[4],
                                          QuadConstruct& quadPoints) {
    ...
    Point strokeCopy[3] = {quadPoints.quad[0], quadPoints.quad[1],
                           quadPoints.quad[2]};
    Point ray[2] = {ray0, ray1};
    return strokeCloseEnough(strokeCopy, ray, quadPoints);
}

void PathStroker::cubicQuadEnds(const Point cubic[4],
                                 QuadConstruct& quadPoints) {
    if (!quadPoints.startSet) {
        Point cubicStartPt = Point::zero();
        cubicPerpRay(cubic, quadPoints.startT, cubicStartPt,
                     quadPoints.quad[0], &quadPoints.tangentStart);
        quadPoints.startSet = true;
    }

    if (!quadPoints.endSet) {
        Point cubicEndPt = Point::zero();
        cubicPerpRay(cubic, quadPoints.endT, cubicEndPt, quadPoints.quad[2],
                     &quadPoints.tangentEnd);
        quadPoints.endSet = true;
    }
}

void PathStroker::closeContour(bool isLine) {
    finishContour(true, isLine);
}

void PathStroker::finishContour(bool close, bool currIsLine) {
    if (segmentCount_ > 0) {
        if (close) {
            (joiner_)(prevUnitNormal_, prevPt_, firstUnitNormal_, radius_,
                      invMiterLimit_, prevIsLine_, currIsLine, builders());
            outer_.close();
            ...
            // cap the end
            auto pt = inner_.lastPoint().value_or(Point::zero());
            const PathBuilder* otherPath =
                currIsLine ? &inner_ : nullptr;
            (capper_)(prevPt_, prevNormal_, pt, otherPath, outer_);
            outer_.reversePathTo(inner_);

            // cap the start
            const PathBuilder* otherPath2 =
                prevIsLine_ ? &inner_ : nullptr;
            (capper_)(firstPt_, -firstNormal_, firstOuterPt_, otherPath2,
                      outer_);
            outer_.close();
        }
...
bool PathStroker::preJoinTo(Point p, bool currIsLine, Point& normal,
                             Point& unitNormal) {
    float prevX = prevPt_.x;
    float prevY = prevPt_.y;

    bool normalSet = setNormalUnitNormal(prevPt_, p, resScale_, radius_,
                                         normal, unitNormal);
    if (!normalSet) {
    ...
    if (segmentCount_ == 0) {
        ...
        (joiner_)(prevUnitNormal_, prevPt_, unitNormal, radius_,
                  invMiterLimit_, prevIsLine_, currIsLine, builders());
    }
...
void PathStroker::initQuad(StrokeType strokeType, NormalizedF32 start,
                            NormalizedF32 end, QuadConstruct& quadPoints) {
...
bool PathStroker::quadStroke(const Point quad[3], QuadConstruct& quadPoints) {
    ...
    if (resultType == ResultType::Quad) {
        ...
        path->quadTo(quadPoints.quad[1].x, quadPoints.quad[1].y,
                     quadPoints.quad[2].x, quadPoints.quad[2].y);
        return true;
    }
...
ResultType PathStroker::compareQuadQuad(const Point quad[3],
                                         QuadConstruct& quadPoints) {
    // get the quadratic approximation of the stroke
    if (!quadPoints.startSet) {
        Point quadStartPt = Point::zero();
        quadPerpRay(quad, quadPoints.startT, quadStartPt, quadPoints.quad[0],
    ...
    if (!quadPoints.endSet) {
        ...
        quadPerpRay(quad, quadPoints.endT, quadEndPt, quadPoints.quad[2],
                    &quadPoints.tangentEnd);
        quadPoints.endSet = true;
    }
...
void PathStroker::setRayPoints(Point tp, Point& dxy, Point& onP,
                                Point* tangent) {
...
void PathStroker::quadPerpRay(const Point quad[3], NormalizedF32 t, Point& tp,
...
ResultType PathStroker::strokeCloseEnough(const Point stroke[3],
                                           const Point ray[2],
                                           QuadConstruct& quadPoints) {
    ...
    NormalizedF32Exclusive roots[3] = {NormalizedF32Exclusive::HALF,
                                       NormalizedF32Exclusive::HALF,
                                       NormalizedF32Exclusive::HALF};
    ...
    Point quadPt =
        path_geometry::evalQuadAt(stroke, roots[0].toNormalized());
    float error =
        invResScale_ * (1.0f - std::abs(roots[0].get() - 0.5f) * 2.0f);
...
ResultType PathStroker::intersectRay(IntersectRayType intersectRayType,
                                      QuadConstruct& quadPoints) {
    ...
    if (validDivide) {
        if (intersectRayType == IntersectRayType::CtrlPt) {
            quadPoints.quad[1].x =
                start.x * (1.0f - numerA) +
                quadPoints.tangentStart.x * numerA;
            quadPoints.quad[1].y =
                start.y * (1.0f - numerA) +
                quadPoints.tangentStart.y * numerA;
        }
        return ResultType::Quad;
    }
...
ResultType PathStroker::tangentsMeet(const Point cubic[4],
                                      QuadConstruct& quadPoints) {
    cubicQuadEnds(cubic, quadPoints);
...
bool PathStroker::hasOnlyMoveTo() const {
    return segmentCount_ == 0;
}
