...
struct DashParams {
  ...
  [[nodiscard]] static std::optional<DashParams> create(
...
struct ContourMeasure {
  ...
  void pushSegment(float startD, float stopD, bool startWithMoveTo,
                   PathBuilder& pb) const;

  /// Map a distance value to a (segmentIndex, NormalizedF32 t) pair.
  [[nodiscard]] std::optional<std::pair<std::size_t, NormalizedF32>>
  distanceToSegment(float distance) const;

  /// Measure a line from p0 to p1, appending a segment if the distance grows.
  float computeLineSeg(Point p0, Point p1, float distance,
                       std::size_t pointIndex);

  /// Recursively measure a quadratic, subdividing until flat enough.
  float computeQuadSegs(Point p0, Point p1, Point p2, float distance,
  ...
  float computeCubicSegs(Point p0, Point p1, Point p2, Point p3,
                         float distance, std::uint32_t minT,
                         std::uint32_t maxT, std::size_t pointIndex,
                         float tolerance);
};
...
[[nodiscard]] std::optional<Path> dashImpl(const Path& src,
                                            const StrokeDash& dash,
                                            float resScale);
