...
inline ::testing::Matcher<const tiny_skia::Point&> PointEq(float x, float y) {
  ...
  return AllOf(Field("x", &tiny_skia::Point::x, FloatEq(x)),
               ...
               ResultOf("is finite", [](const tiny_skia::Point& p) {
                 return p.x == p.x && p.y == p.y;
               }, Eq(true)));
}

inline ::testing::Matcher<const tiny_skia::PathEdge&> PathEdgeLineEq(float x0,
                                                                      float y0,
                                                                      float x1,
                                                                      float y1) {
  using ::testing::AllOf;
  ...
  return AllOf(Field("type", &tiny_skia::PathEdge::type, Eq(tiny_skia::PathEdgeType::LineTo)),
               ResultOf("p0", [](const tiny_skia::PathEdge& edge) {
                 return edge.points[0];
               }, PointEq(x0, y0)),
               ResultOf("p1", [](const tiny_skia::PathEdge& edge) {
...
inline ::testing::Matcher<const tiny_skia::PathEdge&> VerticalPathEdgeAtX(float x) {
  ...
  return AllOf(ResultOf("p0.x", [](const tiny_skia::PathEdge& edge) {
                 return edge.points[0].x;
               }, FloatEq(x)),
               ResultOf("p1.x", [](const tiny_skia::PathEdge& edge) {
                 return edge.points[1].x;
               }, FloatEq(x)),
               ResultOf("line type", [](const tiny_skia::PathEdge& edge) {
