...
TEST(FloatingPointTest, NormalizedF32CreateAndClamp) {
  ...
  EXPECT_EQ(
      tiny_skia::NormalizedF32::create(std::numeric_limits<float>::infinity()),
      std::nullopt);
  EXPECT_EQ(
      tiny_skia::NormalizedF32::create(std::numeric_limits<float>::quiet_NaN()),
      std::nullopt);
...
TEST(FloatingPointTest, NormalizedF32ExclusiveAndStrictWrappers) {
  ...
  EXPECT_NEAR(
      tiny_skia::NormalizedF32Exclusive::newBounded(10.0f).get(),
  ...
  EXPECT_EQ(
      tiny_skia::NonZeroPositiveF32::create(std::numeric_limits<float>::infinity()),
      std::nullopt);

  ASSERT_TRUE(tiny_skia::FiniteF32::create(0.0f).has_value());
  EXPECT_EQ(tiny_skia::FiniteF32::create(std::numeric_limits<float>::infinity()),
            std::nullopt);
  EXPECT_EQ(
      tiny_skia::FiniteF32::create(std::numeric_limits<float>::quiet_NaN()),
      std::nullopt);
}
...
TEST(FloatingPointTest, SaturatingCastsMatchRustSemantics) {
  ...
  EXPECT_EQ(tiny_skia::saturateCastI32(-1.9f), -1);
  EXPECT_EQ(
      tiny_skia::saturateCastI32(std::numeric_limits<float>::quiet_NaN()),
      kMaxI32FitsInF32);
  EXPECT_EQ(
      tiny_skia::saturateCastI32(std::numeric_limits<float>::infinity()),
      kMaxI32FitsInF32);
  EXPECT_EQ(
      tiny_skia::saturateCastI32(-std::numeric_limits<float>::infinity()),
      kMinI32FitsInF32);
  ...
  EXPECT_EQ(
      tiny_skia::saturateCastI32(std::numeric_limits<double>::quiet_NaN()),
      std::numeric_limits<std::int32_t>::max());
  EXPECT_EQ(
      tiny_skia::saturateCastI32(std::numeric_limits<double>::infinity()),
      std::numeric_limits<std::int32_t>::max());
  EXPECT_EQ(
      tiny_skia::saturateCastI32(-std::numeric_limits<double>::infinity()),
...
TEST(FloatingPointTest, SaturatingRoundHelpersMatchTrait) {
  ...
  EXPECT_EQ(tiny_skia::saturateRoundToI32(-1.2f), -1);
  EXPECT_EQ(
      tiny_skia::saturateRoundToI32(std::numeric_limits<float>::quiet_NaN()),
      2147483520);
}

TEST(FloatingPointTest, F32TwoSComplimentOrdering) {
  EXPECT_EQ(tiny_skia::f32As2sCompliment(-0.0f),
            tiny_skia::f32As2sCompliment(0.0f));
  EXPECT_LT(tiny_skia::f32As2sCompliment(-2.0f),
            tiny_skia::f32As2sCompliment(-1.0f));
  EXPECT_LT(tiny_skia::f32As2sCompliment(-1.0f),
            tiny_skia::f32As2sCompliment(0.0f));
  EXPECT_LT(tiny_skia::f32As2sCompliment(0.0f),
            tiny_skia::f32As2sCompliment(1.0f));
}
