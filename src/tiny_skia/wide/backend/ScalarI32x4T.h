...
[[nodiscard]] inline std::array<std::int32_t, 4> i32x4CmpEq(const std::array<std::int32_t, 4>& lhs,
                                                             const std::array<std::int32_t, 4>&
                                                                 rhs) {
  return {i32x4CmpMask(lhs[0] == rhs[0]), i32x4CmpMask(lhs[1] == rhs[1]),
          i32x4CmpMask(lhs[2] == rhs[2]), i32x4CmpMask(lhs[3] == rhs[3])};
}

[[nodiscard]] inline std::array<std::int32_t, 4> i32x4CmpGt(const std::array<std::int32_t, 4>& lhs,
                                                             const std::array<std::int32_t, 4>&
                                                                 rhs) {
  return {i32x4CmpMask(lhs[0] > rhs[0]), i32x4CmpMask(lhs[1] > rhs[1]),
          i32x4CmpMask(lhs[2] > rhs[2]), i32x4CmpMask(lhs[3] > rhs[3])};
...
[[nodiscard]] inline std::array<std::int32_t, 4> i32x4CmpLt(const std::array<std::int32_t, 4>& lhs,
                                                             const std::array<std::int32_t, 4>&
                                                                 rhs) {
  return {i32x4CmpMask(lhs[0] < rhs[0]), i32x4CmpMask(lhs[1] < rhs[1]),
          i32x4CmpMask(lhs[2] < rhs[2]), i32x4CmpMask(lhs[3] < rhs[3])};
...
[[nodiscard]] inline std::array<float, 4> i32x4ToF32(const std::array<std::int32_t, 4>& lanes) {
  return {static_cast<float>(lanes[0]), static_cast<float>(lanes[1]),
          static_cast<float>(lanes[2]), static_cast<float>(lanes[3])};
}
