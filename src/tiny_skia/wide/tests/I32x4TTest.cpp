...
TEST(I32x4TTest, ComparisonsEmitMinusOneMasks) {
  ...
  EXPECT_EQ(lhs.cmpEq(rhs).lanes(), (std::array<std::int32_t, 4>{-1, 0, -1, 0}));
  EXPECT_EQ(lhs.cmpGt(rhs).lanes(), (std::array<std::int32_t, 4>{0, -1, 0, 0}));
  EXPECT_EQ(lhs.cmpLt(rhs).lanes(), (std::array<std::int32_t, 4>{0, 0, 0, -1}));
}
...
TEST(I32x4TTest, BlendUsesMaskBitsPerLane) {
  ...
  EXPECT_EQ(mask.blend(onTrue, onFalse).lanes(),
            (std::array<std::int32_t, 4>{10, 2, 30, 4}));
}
...
TEST(I32x4TTest, AddAndMulUseWrappingSemantics) {
  ...
  EXPECT_EQ((lhs + rhs).lanes(),
            (std::array<std::int32_t, 4>{-2147483648, 1, 92682, 130000}));
  EXPECT_EQ((lhs * rhs).lanes(),
            (std::array<std::int32_t, 4>{2147483647, -2, -2147479015, -1294967296}));
}
...
TEST(I32x4TTest, ToF32x4AndBitcastMatchRustConversions) {
  ...
  EXPECT_EQ(numeric.lanes(), (std::array<float, 4>{1.0f, -2.0f, 3.0f, -4.0f}));
