...
inline std::size_t chopQuadAtXExtrema(std::array<Point, 3> src, std::array<Point, 5>& dst) {
  return ::tiny_skia::path_geometry::chopQuadAtXExtrema(src, dst);
}

inline std::size_t chopQuadAtYExtrema(std::array<Point, 3> src, std::array<Point, 5>& dst) {
  return ::tiny_skia::path_geometry::chopQuadAtYExtrema(src, dst);
}

inline std::size_t chopCubicAtXExtrema(std::array<Point, 4> src, std::array<Point, 10>& dst) {
  return ::tiny_skia::path_geometry::chopCubicAtXExtrema(src, dst);
}

inline std::size_t chopCubicAtYExtrema(std::array<Point, 4> src, std::array<Point, 10>& dst) {
...
inline bool chopMonoCubicAtX(std::array<Point, 4> src, float x, std::array<Point, 7>& dst) {
  return ::tiny_skia::path_geometry::chopMonoCubicAtX(src, x, dst);
}

inline bool chopMonoCubicAtY(std::array<Point, 4> src, float y, std::array<Point, 7>& dst) {
